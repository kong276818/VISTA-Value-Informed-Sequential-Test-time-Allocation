...
~\cite{wang2026conformalthinking}, and supervised adaptive compute allocation
~\cite{zhai2026adaptive}. VISTA does not argue that these signals or controls
...
A note on operating points: at $\lambda=0$, the prequential monitor
correctly abstains from switching---the CS declines to certify any cheaper
policy as better in expectation when only ${\approx}40$ audited items are
available per cell at $n=200$, $\rho=0.20$.
This means all monitoring-variant rows of
Table~\ref{tab:a20_component_removal} deploy the same policy by
construction, and deployment metrics are therefore identical across those
rows.
The statistical components (IPW, CS) must be measured where they directly
affect estimation quality: IPW reduces bias by $2.9\times$--$33\times$
...
additional variance and raises the wrong-best rate (M1/DS1: 23\%; M2/DS2: 60\%;
M2/DS4: 53\%).
...
distributions) will be released at [URL] upon acceptance under [license].
...
Table~\ref{tab:a20_component_removal} summarises the component-removal
analysis for M1/MMLU-Pro.
...
\multicolumn{5}{@{}l}{
\textit{Monitoring components
(no policy switch at $\lambda=0$)}
} \\
...
\multicolumn{5}{@{}l}{
\textit{Monitoring effects measured in dedicated ablations}
} \\

IPW bias reduction
 & \multicolumn{4}{l}{
 $2.9\times$--$33\times$ across cells
 (Table~\ref{tab:a01_ipw_naive})
 } \\

CS FPR control
 & \multicolumn{4}{l}{
 FPR $=0.000$ vs.\ greedy $0$--$58.5\%$
 (Table~\ref{tab:a03_cs_greedy})
 } \\
...
$\rho=0.20$, $n=1000$).
At $\lambda=0$ the confidence sequence certifies no policy switch, so all
monitoring-variant rows deploy the identical policy and share identical
deployment metrics by construction.
The statistical contributions of IPW and CS are therefore measured as
estimation bias (Table~\ref{tab:a01_ipw_naive}) and false-switch rate
(Table~\ref{tab:a03_cs_greedy}) respectively, not as deployment metrics.
This table is a decomposition of the deployed policy, not a component ablation.
The \textit{VISTA ($\lambda=0$) --- no switch certified} row matches
Table~\ref{tab:main} row `VISTA ($\lambda{=}0$, $\rho{=}0.20$)' (Brier $0.492$,
$8191$ tokens, net saving $0.000$).
}
