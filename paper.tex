...
\documentclass[a4paper,fleqn,singleblind]{cas-sc}
...
%% -----------------------------------------------------------------------
\begin{abstract}
When deployment data alone drive reasoning-budget selection,
naive policy comparison is dangerously unreliable:
we show that omitting inverse-probability weighting reverses policy rankings
in five of eight experimental cells (wrong-best rate $1.00$ in those cells),
because early stopping censors the outcomes that longer budgets would have produced.
This finding motivates \textbf{VISTA} (Value-Informed Sequential Test-time
Allocation), a prequential budget-selection procedure that combines
randomized full-trajectory audits (fraction $\rho$ of queries) to recover
censored counterfactuals, importance-weighted policy comparison to correct
selection bias, and anytime-valid confidence sequences to prevent spurious
switching under continuous monitoring---all without a held-out calibration set.
...
best fixed budget in all Gate-0-passing cells, achieving $\approx 0\%$ net
...
%% -----------------------------------------------------------------------
\begin{highlights}
\item Naive policy comparison from deployment data reverses rankings in 5 of 8
cells (wrong-best rate 1.00); inverse-probability weighting reduces bias
$2.2\times$--$23\times$ across all cells.

\item Full-corpus audit across four QA benchmarks reveals qualitatively
heterogeneous budget-response curves: near-linear, early-saturating, and
near-monotone degradation---ruling out a universal fixed allocation.

\item VISTA---combining randomized audits, IPW policy comparison, and
anytime-valid confidence sequences---achieves $3$--$59\%$ net token savings
(at $\lambda{=}0.01$--$0.1$) with Brier increases $\leq 0.019$, without
any held-out calibration set.

\item At $\lambda{=}0$ (quality-preserving), VISTA correctly certifies no
adaptation when no cheaper policy is verifiably better---a principled null
result, not a framework failure.
\end{highlights}
...
(Proposition~\ref{prop:impossibility}): for any estimator that is
...
\begin{enumerate}
\item \textbf{IPW necessity---the strongest empirical finding.}
We show that omitting inverse-probability weighting reverses policy rankings
...
\begin{remark}
Proposition~\ref{prop:impossibility} is a standard label-free learning
...
The author thanks the anonymous reviewers for their constructive criticism,
which materially improved the clarity of the empirical claims and the
treatment of the asymptotic gap-narrowing argument.
...
used.
The author gratefully acknowledges the Qwen team at Alibaba Cloud for
