...
\begin{center}
  \includegraphics[width=\textwidth, height=0.55\textheight, keepaspectratio=true]{img/img1.png}
\end{center}
\begingroup
...
M2 DS1 $76.5\%$, DS2 $85.2\%$, DS3 $85.8\%$, DS4 $81.5\%$.
...
The constrained matched-mean-quality oracle (Oracle~C) passing Gate~0 in six
of eight cells with savings $86$--$97\%$ relative to best fixed is the
operative matched-quality result (Table~\ref{tab:gate0-oracle-c}).
Per-item heterogeneity is substantial even in cells where the mean trajectory
is weakly or inversely responsive to budget.

\input{tables/gate0_oracle_c}
...
VISTA ($\lambda{=}0$) is quality-preserving: the CS correctly certifies no
cheaper policy, yielding 0\% net saving.
VISTA ($\lambda{=}0.01$) trades $+0.6$ pp Brier for 3.2\% net token saving
(200 permutations, $\rho{=}0.20$, $\alpha{=}0.05$).}
\label{tab:main}
...
\midrule
VISTA ($\lambda{=}0$, $\rho{=}0.20$)   & 8191 & 0.492 & 2.764 & 0.738 & No  \\
VISTA ($\lambda{=}0.01$, $\rho{=}0.20$) & 7931 & 0.498 & 2.767 & 0.734 & No  \\
\bottomrule
\end{tabular}
\end{table}
...
At $\lambda{=}0$, VISTA in the six Gate-0-passing cells achieves $\approx 0\%$
net saving in cells whose best fixed budget is $b_{\max}{=}8192$ (M1/DS1,
M1/DS2, M1/DS4) and negative net saving in cells where
$b_{\mathrm{ref}} \ll b_{\max}$ (M1/DS3: $-142\%$; M2/DS1: $-20\%$;
M2/DS2: $-60\%$).
The negative values are structurally expected: audited items run to
$b_{\max}{=}8192$ while the policy selects a much smaller $b_{\mathrm{ref}}$,
so audit overhead dominates policy savings at $\lambda{=}0$.
These cells require $\lambda{>}0$ to make VISTA deployment-beneficial; the
optimal $\lambda$ for each cell represents a business decision about the
acceptable quality--compute tradeoff.

At $\lambda{=}0.01$, net savings in the three $b_{\mathrm{ref}}{=}8192$ cells
are 3.2\% (M1/DS1), 34.5\% (M1/DS2), and 7.0\% (M1/DS4) with Brier
increases of $+0.006$, $+0.008$, and $+0.012$ respectively.
...
The oracle headroom (86--97\% for these cells) substantially exceeds the
...
in all eight cells ($+0.047$ to $+0.218$ versus best fixed budget), because
more thinking reliably improves calibration even for items where the model
appears confident at a short budget.
Full VISTA deployment resolves the open measurement: at $\lambda{=}0$, deployed
saving is $\approx 0\%$ in all six Gate-0-passing cells (correct conservative
behaviour), while Oracle~C headroom remains $74$--$97\%$; the gap reflects the
$\sim$40 audits available at $n{=}200$, $\rho{=}0.20$.
At $\lambda{=}0.01$, cells with $b_{\mathrm{ref}}{=}8192$ deliver 3--35\% net
saving with Brier increases of $0.006$--$0.012$, confirming that the framework
is operational for explicit quality--compute tradeoffs.
The two cells without Gate-0 opportunity are a genuine negative result:
their Brier monotonically worsens above $b{=}256$, and no policy family can
improve on the minimum budget without Brier degradation.
